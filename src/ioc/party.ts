...
const party = new ContainerModule((bind:interfaces.Bind) => {
    ...
    bind<interfaces.Newable<PackageControllerInterface>>(TYPES.PackageController).toConstructor(PackageController)
    bind<interfaces.Factory<PackageControllerInterface>>(TYPES.PackageFactory)
        .toFactory<PackageControllerInterface>(serviceFactoryFunction<PackageControllerInterface>(TYPES.PackageController))
})
